...
distances_X = 10 + (40)*rand(1, numTx); % 感知目标距离 50-100米

% 生成多径衰落信道 H (numRx x numTx)
H = (randn(numRx, numTx) + 1j*randn(numRx, numTx)) / sqrt(pathLossFactor); 
    ...
    for idx = 1:length(snrRange)
        ...
        for symbolIdx = 1:2:numSymbols-1
            ...
            for j = symbolIdx:symbolIdx+1
                ...
                % [txSignal_upsampled, t] = pulse_shaping_and_upconvert_no_filter(ISAC_signal, fc, fs, samplesPerSymbol);
                ...
                % [rxSignal_processed, rxSymbols] = downconvert_and_filter_no_filter(rxSignal, fc, fs, samplesPerSymbol, numTx);
                ...
                decoded_symbols_map = (qammod(decoded_symbols,M)/sqrt(2))';
        ...
        % 存储结果
        mseErrors_proposed(idx) = mseErrors_proposed(idx) + sensingErrors / (numSymbols/2);
...
function [txSignal_upsampled, t] = pulse_shaping_and_upconvert_no_filter(baseband_signal, fc, fs, samplesPerSymbol)
    % 去除脉冲成形版本，仅做上采样和上变频
    [numTx, numSymbols] = size(baseband_signal);

    % 上采样：为每个符号插入采样点
    total_samples = numSymbols * samplesPerSymbol;
    txSignal_upsampled_baseband = zeros(numTx, total_samples);

    for tx_idx = 1:numTx
        % 在每个符号位置插入值，其他为0（零插值）
        for sym_idx = 1:numSymbols
            sample_idx = (sym_idx - 1) * samplesPerSymbol + 1;
            txSignal_upsampled_baseband(tx_idx, sample_idx) = baseband_signal(tx_idx, sym_idx);
        end
    end
    ...
    % 上变频
    txSignal_upsampled = zeros(numTx, total_samples);
    for i = 1:numTx
        txSignal_upsampled(i,:) = txSignal_upsampled_baseband(i,:) .* exp(1j * 2 * pi * fc * t);
    end
end
function rxSignal = channel_transmission(txSignal, H_channel, noisePower, pathLossFactor)
    ...
    % 添加噪声
    noise = sqrt(noisePower) * (randn(size(rxSignal)) + 1j*randn(size(rxSignal)));
...
function [rxSignal_processed, rxSymbols] = downconvert_and_filter_no_filter(rxSignal, fc, fs, samplesPerSymbol, numTx)
    % 简化版：下变频 + 直接采样，无匹配滤波
    [numRx, numSamples] = size(rxSignal);
    ...
    % 下变频（保留复数）
    rxSignal_downconv = rxSignal .* exp(-1j * 2 * pi * fc * t);

    % 提取符号（直接下采样）
    numSymbols = floor(numSamples / samplesPerSymbol);
    symbol_indices = (0:numSymbols-1) * samplesPerSymbol + 1;

    % 提取对应的采样点
    rxSymbols = zeros(numRx, numTx);
    for rx = 1:numRx
        % 取前 numTx 个符号用于后续估计（你当前只用3个符号）
        rxSymbols(rx, :) = rxSignal_downconv(rx, symbol_indices(1:numTx));
    end

    % 输出处理信号（无滤波，直接下变频）
    rxSignal_processed = rxSignal_downconv;

    % Debug info
    fprintf('[NO-FILTER] Total samples: %d\n', numSamples);
    fprintf('[NO-FILTER] Symbol indices: [%s]\n', num2str(symbol_indices(1:numTx)));
    fprintf('[NO-FILTER] Final rxSymbols size: [%d, %d]\n', size(rxSymbols,1), size(rxSymbols,2));
end
